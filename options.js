...
const providers = {
  ...
  sambanova: { label: "SambaNova API Key", url: "https://api.sambanova.ai/v1/models" },
  cerebras: { label: "Cerebras API Key", url: "https://api.cerebras.ai/v1/models" },
  nvidia: { label: "Nvidia API Key", url: "https://integrate.api.nvidia.com/v1/models" },
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('easySelection').checked = data.easySelection || false;
...
document.getElementById('saveBtn').onclick = async () => {
  ...
  const settings = {
    ...
    easySelection: document.getElementById('easySelection').checked,
  ...
  tabs.forEach(t => chrome.tabs.sendMessage(t.id, { action: "global_state_update", state: settings.extensionEnabled }).catch(() => {}));

  const btn = document.getElementById('saveBtn');
  btn.innerText = "Saved!"; btn.style.background = "#10b981"; btn.style.color = "#fff";
  setTimeout(() => { btn.innerText = "Save Settings"; btn.style.background = ""; btn.style.color = ""; }, 1500);
};
...
document.getElementById('importBtn').onclick = () => {
  ...
  input.onchange = (e) => {
    ...
    reader.onload = async (re) => { await chrome.storage.local.set(JSON.parse(re.target.result)); location.reload(); };
